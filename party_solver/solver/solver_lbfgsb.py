...
from party_solver.tools import wolfe
import sympy as sp
...
def line_search(x0, f0, g0, d, above_iter, max_steplength, \
    ...
    while i < max_iter:
        # steplength, f0, dphi, task = scipy.optimize.minpack2.dcsrch(steplength_0, f_m1, dphi_m1,
        #                                                             alpha, beta, xtol_minpack, task,
        #                                                             0, max_steplength, isave, dsave)
        # print(task)
        # print(steplength)
        steplength, f0, dphi, task = wolfe.dcsrch(steplength_0, f_m1, dphi_m1,
                                                   alpha, beta, xtol_minpack, task,
                                                   0, max_steplength, isave, dsave)
        # print(task)
        # print(steplength)
...
def L_BFGS_B(x0, f, df, l, u, m=10, \
             epsg=1e-5, epsf=1e7, max_iter=500, \
    ...
    while np.max(np.abs(np.clip(x - g, l, u) - x)) > epsg and i < max_iter:
        ...
        if steplength == None:
            ...
            if ((oldf0 - f0) / max(abs(oldf0), abs(f0), 1) < epsmch * epsf):
                print(f"L-BFGS-B optimization convergence achieved after {i} iterations")
                break
            i += 1
...
def l_bfgs_b_optimization(f, vars, x0, epsilon=1e-5, max_iter=1000, m=10, bounds=None):
    """
    使用 L-BFGS-B 进行优化。
    参数:
    f: 待优化的函数。
    ...
    l = np.array([b[0] if b[0] is not None else -np.inf for b in bounds])
    u = np.array([b[1] if b[1] is not None else np.inf for b in bounds])

    f_lamda = sp.lambdify(vars, f, 'numpy')

    def f_wrapper(x):
        return f_lamda(*x)

    def df_wrapper(x):
        return np.array(difftools.gradient(f, vars, x))

    result = L_BFGS_B(x0, f_wrapper, df_wrapper, l, u, epsg=epsilon, max_iter=max_iter, m=m)
    return result['x']
...
if __name__ == "__main__":
    ...
    x0 = np.array([1.8, 1.0, 1.0])
    bounds = [(0,-1), (0,-1), (0,-1)]
    optimal_x = l_bfgs_b_optimization(f_expr, (x, y, z), x0, bounds=bounds)
